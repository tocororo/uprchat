...
async def create_new_llmq(
    chat_id: int,
    session: Annotated[AsyncSession, Depends(get_session)], llmquery: LLMQueryCreate
):
...
async def get_all_llmqueries(chat_id: int,session: Annotated[AsyncSession, Depends(get_session)],offset: int = 0, limit: int = 100):
...
async def get_one_llmquery(id: int,session: Annotated[AsyncSession, Depends(get_session)]):
...
async def update_one_llmquery(id: int,llmquery_update: LLMQueryUpdate,session: Annotated[AsyncSession, Depends(get_session)]):
...
async def delete_one_llmquery(id: int,session: Annotated[AsyncSession, Depends(get_session)]):
